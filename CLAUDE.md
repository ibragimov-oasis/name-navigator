...
**Fable 5** = `claude-fable-5` (orchestrator: plans, delegates, reviews — it burns tokens fastest, so it does judgment, not labor) · **Opus 4.8** = `claude-opus-4-8` (hard reasoning, architecture, security; also the fallback when Fable declines a task) · **Sonnet 5** = `claude-sonnet-5` (mechanical/bulk/search-heavy work, repetitive refactors, live E2E browser checks). **Haiku is BANNED (user rule 2026-07-02): never route any task to `haiku` / `claude-haiku-4-5-*`; wherever haiku would have been used, use Sonnet 5 instead.** Subagent `model` param takes aliases `fable | opus | sonnet` (never `haiku`); omit to inherit the session model.
- **Verification — never self-review.** The agent that wrote the work never checks it. Spawn a cold `sonnet` verifier with zero context of the implementation — goal + result only. (Never `haiku` — banned.)
